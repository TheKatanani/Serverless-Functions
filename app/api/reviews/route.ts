...
import { getBooks, getReviews, saveReviews, Review } from '@/app/lib/data';
...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!bookId || !author || !rating || !title || !comment) {
            return NextResponse.json({ message: 'Missing required fields for a new review.' }, { status: 400 });
        }
        ...
        if (!bookExists) {
            return NextResponse.json({ message: `Cannot add review: Book with ID ${bookId} not found.` }, { status: 404 });
        }
